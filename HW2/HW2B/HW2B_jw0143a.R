...
# OBSERVATIONS:
# It is possible to tell which play is the tragedy and which one is the comedy,
# observing that in the graphical analysis, negative sentiment is greater for
# Hamlet, and positive sentiment is greater for Love's Labour Cost. The
# ratio between negative and positive is not quite significant, though. The
# absolute values of the standard residual associated with the chi-square test 
# are over 9 and a half, indicating that there are great outliers.
...
tidy_book_sentimental <- tidy_book_sentimental %>%
  inner_join(shakespeare_corpus, by=c('gutenberg_id'="Gutenberg ID"))

tidy_book_sentimental %>% group_by(Type) %>% 
  summarize(value = mean(value))

tidy_book_sentimental %>% ggplot(aes(Type, value)) + geom_jitter()
